...
local function Buffer(length)
	...
	obj.buffer = new('unsigned char[?]', obj.capacity)

	if data then
		copy(obj.buffer, data, length)
	end
...
function BUFFER:__tostring()
	return format("Buffer[%d/%d]", self.length, self.capacity)
end
...
function BUFFER:toHex()
	return (gsub(self:toString(), '.', function (c)
...
BUFFER.getLength = BUFFER.__len

function BUFFER:writeBuffer(buffer)
	self:write(buffer:toString())
end
...
function BUFFER:write(data)
	...
	-- If we are writing data somewhere in the middle..
	if self.position < self.length then
		-- Shift data so it doesn't get overwritten by the copy below
		copy(self.buffer + self.position + len, self.buffer + self.position, self.length)
	end

	-- Write at the postion
	copy(self.buffer + self.position, data, len)
...
function BUFFER:dump(buf)
	...
	for i = 0, ceil(len/10)-1 do
		for x = i * 10, min((i+1)*10, len)-1 do
			io.write(format('%.2x', self.buffer[x]) .. ' ')
		end
